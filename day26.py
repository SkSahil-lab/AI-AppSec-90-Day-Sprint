...
class PromptInjectionParser:
    ...
    def parse_and_sanitize(self, input_text: str, current_depth: int = 0, max_depth: int = 3) -> Dict[str, Any]:
        """
        Recursively flattens and normalizes input blocks to detect nested context overrides.
        """
        start_time = time.perf_counter()
        normalized_text = input_text.lower().strip()
       
        if current_depth > max_depth:
            return {"verdict": "REJECTED", "reason": "Maximum recursion depth breached (Stack Exploit Risk)"}

        for signature in self.jailbreak_signatures:
            if signature in normalized_text:
                return {"verdict": "REJECTED", "reason": f"OWASP-LLM01: Malicious instruction sequence detected ['{signature}']"}
        ...
        return {
            "verdict": "🟢 Passed",
            "overhead_seconds": execution_time
        }
