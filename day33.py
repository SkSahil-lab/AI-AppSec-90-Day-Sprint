...
class RAGContextGuard:
    def __init__(self):
        # Scan for instruction-forcing verbs and system override flags hidden in context files
    ...
    def sanitize_retrieved_context(self, context_chunks: List[str]) -> Dict[str, Any]:
        """
        Audits text fragments retrieved from external vector databases to prevent Indirect Prompt Injections.
        """
        ...
        # Enforce an explicit slice window cap to prevent massive database dumps from causing DoS
        safe_context_pool = context_chunks[:10]

        for index, chunk in enumerate(safe_context_pool):
            normalized_chunk = chunk.strip()
            
            # Step 1: Scan individual vector snippets for context breakout indicators
            if self.indirect_injection_pattern.search(normalized_chunk):
                ...
                # Mitigation Strategy: Drop or neutralize the corrupted chunk data completely
                continue
            
            audited_chunks.append(normalized_chunk)

        execution_time = time.perf_counter() - start_time
        verdict_state = "⚠️ Flagged/Sanitized" if violations_logged else "🟢 Passed"
...
if __name__ == "__main__":
    ...
    # Simulation: A mock database lookup returns clean data alongside a poisoned review document snippet
